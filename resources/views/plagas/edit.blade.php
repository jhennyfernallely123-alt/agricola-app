...
<div class="container">
    <div class="tarjeta-crud">
        <h1><i class="bi bi-pencil"></i> Editar Control de Plagas</h1>
        <form action="{{ route('plagas.update', $control) }}" method="POST" class="form-agricola">
            @csrf @method('PUT')
            <div class="mb-3">
                ...
                <select name="cultivo_id" id="cultivo_id" class="form-select" required>
                    ...
                    <option value="{{ $cultivo->id }}" {{ old('cultivo_id', $control->cultivo_id) == $cultivo->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select name="tipo" id="tipo" class="form-select" required>
                    ...
                    <option value="plaga" {{ old('tipo', $control->tipo) == 'plaga' ? 'selected' : '' }}>Plaga</option>
                    <option value="enfermedad" {{ old('tipo', $control->tipo) == 'enfermedad' ? 'selected' : '' }}>Enfermedad</option>
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nombre" id="nombre" class="form-control" value="{{ old('nombre', $control->nombre) }}" required>
            ...
            <div class="mb-3">
                ...
                <input type="date" name="fecha_deteccion" id="fecha_deteccion" class="form-control" value="{{ old('fecha_deteccion', $control->fecha_deteccion) }}" required>
            ...
            <div class="mb-3">
                ...
                <textarea name="tratamiento_aplicado" id="tratamiento_aplicado" class="form-control" rows="3">{{ old('tratamiento_aplicado', $control->tratamiento_aplicado) }}</textarea>
                @error('tratamiento_aplicado') <div class="text-danger mt-1"><i class="bi bi-exclamation-circle"></i> {{ $message }}</div> @enderror
            </div>
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg"></i> Actualizar Control de Plagas</button>
